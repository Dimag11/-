    ...
    static void Main()
         {
          ...
          #region Присвоение значений переменным для первого из заданий
           ...
           a1 = Math.Pow(Math.Sqrt(7/3), 0.33);
          ...
          #region Вычисление результатов для двух заданий
           result1 = d + i + j + k;
           result2 = (((Math.Pow(Math.Sqrt((2.7 - 0.8) * (Math.Pow(Math.Sqrt(7/3), 0.33))) / (((5.2 - 1.4) * 7.0) / 3.0), 0.2)) + (8.0 * (9.0 / 11.0))) + ((7.0 / (30.0 - 93.0 * (5.0 / 18.0)) + 95.0) * 2.25) / (Math.Pow(2.0, 1.2)) + (((Math.Pow(Math.Sqrt(Math.PI), 0.16)) + (2.0 * Math.PI / 3.0))));
          #endregion
